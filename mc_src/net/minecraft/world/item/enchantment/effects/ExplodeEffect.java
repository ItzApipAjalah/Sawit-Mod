...
public record ExplodeEffect(
	...
	public void apply(ServerLevel arg, int i, EnchantedItemInUse arg2, Entity arg3, Vec3 arg4) {
		Vec3 vec3 = arg4.add(this.offset);
		arg.explode(
			this.attributeToUser ? arg3 : null,
			this.getDamageSource(arg3, vec3),
			new SimpleExplosionDamageCalculator(
				this.blockInteraction != Level.ExplosionInteraction.NONE,
				this.damageType.isPresent(),
				this.knockbackMultiplier.map(argx -> argx.calculate(i)),
				this.immuneBlocks
			),
			vec3.x(),
			vec3.y(),
	...
	@Nullable
	private DamageSource getDamageSource(Entity arg, Vec3 arg2) {
		if (this.damageType.isEmpty()) {
			return null;
		} else {
			return this.attributeToUser
				? new DamageSource((Holder<DamageType>)this.damageType.get(), arg)
				: new DamageSource((Holder<DamageType>)this.damageType.get(), arg2);
		}
	}
