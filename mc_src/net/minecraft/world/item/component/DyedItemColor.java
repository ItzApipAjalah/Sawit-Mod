...
public record DyedItemColor(int rgb, boolean showInTooltip) implements TooltipProvider {
	private static final Codec<DyedItemColor> FULL_CODEC = RecordCodecBuilder.create(
		instance -> instance.group(
				Codec.INT.fieldOf("rgb").forGetter(DyedItemColor::rgb), Codec.BOOL.optionalFieldOf("show_in_tooltip", true).forGetter(DyedItemColor::showInTooltip)
			)
	...
	public static int getOrDefault(ItemStack arg, int i) {
		DyedItemColor dyedItemColor = arg.get(DataComponents.DYED_COLOR);
		return dyedItemColor != null ? ARGB.opaque(dyedItemColor.rgb()) : i;
	}

	public static ItemStack applyDyes(ItemStack arg, List<DyeItem> list) {
		if (!arg.is(ItemTags.DYEABLE)) {
			return ItemStack.EMPTY;
		} else {
			ItemStack itemStack = arg.copyWithCount(1);
			int i = 0;
			int j = 0;
			int k = 0;
			int l = 0;
