...
public record LodestoneTracker(Optional<GlobalPos> target, boolean tracked) {
	...
	public LodestoneTracker tick(ServerLevel arg) {
		if (this.tracked && !this.target.isEmpty()) {
			if (((GlobalPos)this.target.get()).dimension() != arg.dimension()) {
				return this;
			} else {
				BlockPos blockPos = ((GlobalPos)this.target.get()).pos();
				return arg.isInWorldBounds(blockPos) && arg.getPoiManager().existsAtPosition(PoiTypes.LODESTONE, blockPos)
					? this
