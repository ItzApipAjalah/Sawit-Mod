...
public record Tool(List<Tool.Rule> rules, float defaultMiningSpeed, int damagePerBlock) {
	...
	public float getMiningSpeed(BlockState arg) {
		for (Tool.Rule rule : this.rules) {
			if (rule.speed.isPresent() && arg.is(rule.blocks)) {
	...
	public boolean isCorrectForDrops(BlockState arg) {
		for (Tool.Rule rule : this.rules) {
			if (rule.correctForDrops.isPresent() && arg.is(rule.blocks)) {
	...
	public record Rule(HolderSet<Block> blocks, Optional<Float> speed, Optional<Boolean> correctForDrops) {
		...
		public static Tool.Rule minesAndDrops(HolderSet<Block> arg, float f) {
			return new Tool.Rule(arg, Optional.of(f), Optional.of(true));
		}

		public static Tool.Rule deniesDrops(HolderSet<Block> arg) {
			return new Tool.Rule(arg, Optional.empty(), Optional.of(false));
		}

		public static Tool.Rule overrideSpeed(HolderSet<Block> arg, float f) {
			return new Tool.Rule(arg, Optional.of(f), Optional.empty());
		}
	}
}
