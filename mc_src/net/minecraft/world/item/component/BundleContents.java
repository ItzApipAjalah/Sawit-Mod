...
public final class BundleContents implements TooltipComponent {
	...
	public static final Codec<BundleContents> CODEC = ItemStack.CODEC.listOf().flatXmap(BundleContents::checkAndCreate, arg -> DataResult.success(arg.items));
	public static final StreamCodec<RegistryFriendlyByteBuf, BundleContents> STREAM_CODEC = ItemStack.STREAM_CODEC
		.apply(ByteBufCodecs.list())
		.map(BundleContents::new, arg -> arg.items);
	...
	static Fraction getWeight(ItemStack arg) {
		...
		if (bundleContents != null) {
			...
			List<BeehiveBlockEntity.Occupant> list = arg.getOrDefault(DataComponents.BEES, List.of());
			return !list.isEmpty() ? Fraction.ONE : Fraction.getFraction(1, arg.getMaxStackSize());
		}
	}

	public static boolean canItemBeInBundle(ItemStack arg) {
		return !arg.isEmpty() && arg.getItem().canFitInsideContainerItems();
	}
	...
	public static class Mutable {
		...
		public Mutable(BundleContents arg) {
			this.items = new ArrayList(arg.items);
			this.weight = arg.weight;
			this.selectedItem = arg.selectedItem;
		}
		...
		private int findStackIndex(ItemStack arg) {
			if (!arg.isStackable()) {
				return -1;
			} else {
				for (int i = 0; i < this.items.size(); i++) {
					if (ItemStack.isSameItemSameComponents((ItemStack)this.items.get(i), arg)) {
						return i;
					}
				}
		...
		private int getMaxAmountToAdd(ItemStack arg) {
			...
			return Math.max(fraction.divideBy(BundleContents.getWeight(arg)).intValue(), 0);
		}

		public int tryInsert(ItemStack arg) {
			if (!BundleContents.canItemBeInBundle(arg)) {
				return 0;
			} else {
				...
				if (i == 0) {
					...
					this.weight = this.weight.add(BundleContents.getWeight(arg).multiplyBy(Fraction.getFraction(i, 1)));
					int j = this.findStackIndex(arg);
					if (j != -1) {
		...
		public int tryTransfer(Slot arg, Player arg2) {
			...
			return BundleContents.canItemBeInBundle(itemStack) ? this.tryInsert(arg.safeTake(itemStack.getCount(), i, arg2)) : 0;
		}
