...
public record WrittenBookContent(Filterable<String> title, String author, int generation, List<Filterable<Component>> pages, boolean resolved)
	...
	@Nullable
	public WrittenBookContent resolve(CommandSourceStack arg, @Nullable Player arg2) {
		if (this.resolved) {
			return null;
		} else {
			...
			for (Filterable<Component> filterable : this.pages) {
				Optional<Filterable<Component>> optional = resolvePage(arg, arg2, filterable);
				if (optional.isEmpty()) {
					return null;
				}
	...
	private static Optional<Filterable<Component>> resolvePage(CommandSourceStack arg, @Nullable Player arg2, Filterable<Component> arg3) {
		return arg3.resolve(arg3x -> {
			try {
				Component component = ComponentUtils.updateForEntity(arg, arg3x, arg2, 0);
				return isPageTooLarge(component, arg.registryAccess()) ? Optional.empty() : Optional.of(component);
			} catch (Exception var4) {
				return Optional.of(arg3x);
			}
		});
	}

	private static boolean isPageTooLarge(Component arg, HolderLookup.Provider arg2) {
		return Component.Serializer.toJson(arg, arg2).length() > 32767;
	}

	public List<Component> getPages(boolean bl) {
		return Lists.transform(this.pages, arg -> (Component)arg.get(bl));
	}
