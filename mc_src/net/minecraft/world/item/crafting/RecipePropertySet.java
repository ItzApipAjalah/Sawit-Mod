...
public class RecipePropertySet {
	...
	public static final StreamCodec<RegistryFriendlyByteBuf, RecipePropertySet> STREAM_CODEC = ByteBufCodecs.holderRegistry(Registries.ITEM)
		...
		.map(list -> new RecipePropertySet(Set.copyOf(list)), arg -> List.copyOf(arg.items));
	...
	public boolean test(ItemStack arg) {
		return this.items.contains(arg.getItemHolder());
	}

	static RecipePropertySet create(Collection<Ingredient> collection) {
		Set<Holder<Item>> set = (Set<Holder<Item>>)collection.stream().flatMap(arg -> arg.items().stream()).collect(Collectors.toUnmodifiableSet());
