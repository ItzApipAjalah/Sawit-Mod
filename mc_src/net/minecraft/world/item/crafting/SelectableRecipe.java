...
public record SelectableRecipe<T extends Recipe<?>>(SlotDisplay optionDisplay, Optional<RecipeHolder<T>> recipe) {
	public static <T extends Recipe<?>> StreamCodec<RegistryFriendlyByteBuf, SelectableRecipe<T>> noRecipeCodec() {
		return StreamCodec.composite(SlotDisplay.STREAM_CODEC, SelectableRecipe::optionDisplay, arg -> new SelectableRecipe(arg, Optional.empty()));
	}
	...
	public record SingleInputSet<T extends Recipe<?>>(List<SelectableRecipe.SingleInputEntry<T>> entries) {
		...
		public boolean acceptsInput(ItemStack arg) {
			return this.entries.stream().anyMatch(arg2 -> arg2.input.test(arg));
		}

		public SelectableRecipe.SingleInputSet<T> selectByInput(ItemStack arg) {
			return new SelectableRecipe.SingleInputSet<>(this.entries.stream().filter(arg2 -> arg2.input.test(arg)).toList());
		}
