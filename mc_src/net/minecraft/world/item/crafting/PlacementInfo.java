...
public class PlacementInfo {
	...
	public static StackedContents.IngredientInfo<Holder<Item>> ingredientToContents(Ingredient arg) {
		return StackedItemContents.convertIngredientContents(arg.items().stream());
	}

	public static PlacementInfo create(Ingredient arg) {
		if (arg.items().isEmpty()) {
			return NOT_PLACEABLE;
		} else {
			StackedContents.IngredientInfo<Holder<Item>> ingredientInfo = ingredientToContents(arg);
			PlacementInfo.SlotInfo slotInfo = new PlacementInfo.SlotInfo(0);
			return new PlacementInfo(List.of(arg), List.of(ingredientInfo), List.of(Optional.of(slotInfo)));
		}
	}
