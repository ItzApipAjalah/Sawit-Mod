...
public interface SlotDisplay {
	...
	<T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2);

	SlotDisplay.Type<? extends SlotDisplay> type();

	default boolean isEnabled(FeatureFlagSet arg) {
		return true;
	}

	default List<ItemStack> resolveForStacks(ContextMap arg) {
		return this.resolve(arg, SlotDisplay.ItemStackContentsFactory.INSTANCE).toList();
	}

	default ItemStack resolveForFirstStack(ContextMap arg) {
		return (ItemStack)this.resolve(arg, SlotDisplay.ItemStackContentsFactory.INSTANCE).findFirst().orElse(ItemStack.EMPTY);
	}
	...
	public static class AnyFuel implements SlotDisplay {
		...
		@Override
		public <T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2) {
			if (arg2 instanceof DisplayContentsFactory.ForStacks<T> forStacks) {
				FuelValues fuelValues = arg.getOptional(SlotDisplayContext.FUEL_VALUES);
				if (fuelValues != null) {
	...
	public record Composite(List<SlotDisplay> contents) implements SlotDisplay {
		...
		@Override
		public <T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2) {
			return this.contents.stream().flatMap(arg3 -> arg3.resolve(arg, arg2));
		}

		@Override
		public boolean isEnabled(FeatureFlagSet arg) {
			return this.contents.stream().allMatch(arg2 -> arg2.isEnabled(arg));
		}
	}
	...
	public record ItemSlotDisplay(Holder<Item> item) implements SlotDisplay {
		...
		public ItemSlotDisplay(Item arg) {
			this(arg.builtInRegistryHolder());
		}
		...
		@Override
		public <T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2) {
			return arg2 instanceof DisplayContentsFactory.ForStacks<T> forStacks ? Stream.of(forStacks.forStack(this.item)) : Stream.empty();
		}

		@Override
		public boolean isEnabled(FeatureFlagSet arg) {
			return this.item.value().isEnabled(arg);
		}
	}
	...
	public static class ItemStackContentsFactory implements DisplayContentsFactory.ForStacks<ItemStack> {
		...
		public ItemStack forStack(ItemStack arg) {
			return arg;
		}
	}
	...
	public record ItemStackSlotDisplay(ItemStack stack) implements SlotDisplay {
		...
		@Override
		public <T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2) {
			return arg2 instanceof DisplayContentsFactory.ForStacks<T> forStacks ? Stream.of(forStacks.forStack(this.stack)) : Stream.empty();
		}
		...
		@Override
		public boolean isEnabled(FeatureFlagSet arg) {
			return this.stack.getItem().isEnabled(arg);
		}
	}
	...
	public record SmithingTrimDemoSlotDisplay(SlotDisplay base, SlotDisplay material, SlotDisplay pattern) implements SlotDisplay {
		...
		public <T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2) {
			if (arg2 instanceof DisplayContentsFactory.ForStacks<T> forStacks) {
				HolderLookup.Provider provider = arg.getOptional(SlotDisplayContext.REGISTRIES);
				if (provider != null) {
					RandomSource randomSource = RandomSource.create(System.identityHashCode(this));
					List<ItemStack> list = this.base.resolveForStacks(arg);
					if (list.isEmpty()) {
						return Stream.empty();
					}

					List<ItemStack> list2 = this.material.resolveForStacks(arg);
					if (list2.isEmpty()) {
						return Stream.empty();
					}

					List<ItemStack> list3 = this.pattern.resolveForStacks(arg);
					if (list3.isEmpty()) {
						return Stream.empty();
					}
					...
					}).limit(256L).filter(argx -> !argx.isEmpty()).limit(16L).map(forStacks::forStack);
				}
			}
	...
	public record TagSlotDisplay(TagKey<Item> tag) implements SlotDisplay {
		...
		public <T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2) {
			if (arg2 instanceof DisplayContentsFactory.ForStacks<T> forStacks) {
				HolderLookup.Provider provider = arg.getOptional(SlotDisplayContext.REGISTRIES);
				if (provider != null) {
					return provider.lookupOrThrow(Registries.ITEM).get(this.tag).map(arg2x -> arg2x.stream().map(forStacks::forStack)).stream().flatMap(stream -> stream);
				}
			}
	...
	public record WithRemainder(SlotDisplay input, SlotDisplay remainder) implements SlotDisplay {
		...
		@Override
		public <T> Stream<T> resolve(ContextMap arg, DisplayContentsFactory<T> arg2) {
			if (arg2 instanceof DisplayContentsFactory.ForRemainders<T> forRemainders) {
				List<T> list = this.remainder.resolve(arg, arg2).toList();
				return this.input.resolve(arg, arg2).map(object -> forRemainders.addRemainder((T)object, list));
			} else {
				return this.input.resolve(arg, arg2);
			}
		}

		@Override
		public boolean isEnabled(FeatureFlagSet arg) {
			return this.input.isEnabled(arg) && this.remainder.isEnabled(arg);
		}
	}
}
