...
public enum DyeColor implements StringRepresentable {
	...
	private static final Int2ObjectOpenHashMap<DyeColor> BY_FIREWORK_COLOR = new Int2ObjectOpenHashMap<>(
		(Map<? extends Integer, ? extends DyeColor>)Arrays.stream(values()).collect(Collectors.toMap(arg -> arg.fireworkColor, arg -> arg))
	);
	...
	private DyeColor(int j, String string2, int k, MapColor arg, int l, int m) {
		this.id = j;
		this.name = string2;
		this.mapColor = arg;
		this.textColor = m;
		this.dyesTag = ItemTags.create(ResourceLocation.fromNamespaceAndPath("c", "dyes/" + string2));
		this.dyedTag = ItemTags.create(ResourceLocation.fromNamespaceAndPath("c", "dyed/" + string2));
	...
	public static DyeColor byName(String string, @Nullable DyeColor arg) {
		DyeColor dyecolor = (DyeColor)CODEC.byName(string);
		return dyecolor != null ? dyecolor : arg;
	}
