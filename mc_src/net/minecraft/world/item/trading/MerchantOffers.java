...
public class MerchantOffers extends ArrayList<MerchantOffer> {
	...
	@Nullable
	public MerchantOffer getRecipeFor(ItemStack arg, ItemStack arg2, int i) {
		if (i > 0 && i < this.size()) {
			MerchantOffer merchantOffer = (MerchantOffer)this.get(i);
			return merchantOffer.satisfiedBy(arg, arg2) ? merchantOffer : null;
		} else {
			for (int j = 0; j < this.size(); j++) {
				MerchantOffer merchantOffer2 = (MerchantOffer)this.get(j);
				if (merchantOffer2.satisfiedBy(arg, arg2)) {
