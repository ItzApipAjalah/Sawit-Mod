...
public class HoneycombItem extends Item implements SignApplicator {
	...
	public HoneycombItem(Item.Properties arg) {
		super(arg);
	}
	...
	public InteractionResult useOn(UseOnContext arg) {
		...
		return (InteractionResult)getWaxed(blockstate).map(arg2 -> {
			...
			itemstack.shrink(1);
			level.setBlock(blockpos, arg2, 11);
			level.gameEvent(GameEvent.BLOCK_CHANGE, blockpos, GameEvent.Context.of(player, arg2));
			level.levelEvent(player, 3003, blockpos, 0);
			return InteractionResult.SUCCESS;
		}).orElse(InteractionResult.PASS);
	}

	public static Optional<BlockState> getWaxed(BlockState arg) {
		return Optional.ofNullable(DataMapHooks.getBlockWaxed(arg.getBlock())).map(arg2 -> arg2.withPropertiesOf(arg));
	}

	@Override
	public boolean tryApplyToSign(Level arg, SignBlockEntity arg2, boolean bl, Player arg3) {
		if (arg2.setWaxed(true)) {
			arg.levelEvent(null, 3003, arg2.getBlockPos(), 0);
			return true;
		} else {
			return false;
		}
	...
	@Override
	public boolean canApplyToSign(SignText arg, Player arg2) {
		return true;
	}
}
