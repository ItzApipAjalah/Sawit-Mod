...
public class SawitTrunkBlock extends Block implements BonemealableBlock {
    public static final MapCodec<SawitTrunkBlock> CODEC = simpleCodec(SawitTrunkBlock::new);
    ...
    protected InteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hit) {
        if (stack.getItem() instanceof net.minecraft.world.item.AxeItem) {
            ...
            if (!level.isClientSide()) {
                level.setBlock(pos, ModBlocks.STRIPPED_SAWIT_LOG.get().defaultBlockState().setValue(net.minecraft.world.level.block.RotatedPillarBlock.AXIS, state.hasProperty(net.minecraft.world.level.block.RotatedPillarBlock.AXIS) ? state.getValue(net.minecraft.world.level.block.RotatedPillarBlock.AXIS) : net.minecraft.core.Direction.Axis.Y), 11);
